...
const FANDOM_RULE = 'Character identity: if a character is recognisable as an existing character from an anime, game, manga or other work, lead their tag group with the canonical Danbooru character tag written as "name (series)", then the series tag - for example "amiya (arknights), arknights". A known costume variant tag such as "usada pekora (1st costume)" may follow it. Use these only when you are sure of the exact Danbooru tag; if unsure, drop it and describe the character with plain appearance tags instead of guessing. Keep the appearance tags either way.';

const DENSITY_RULE = 'Emphasis: NovelAI numeric emphasis is written as "1.2::tag, tag ::" and must be closed with a bare "::". Use it in 2 or 3 spots at most: raise the focus of the image to 1.15-1.3, lower background clutter or a detail that keeps stealing attention to 0.7-0.9, and use "-1::tag ::" to remove something the character normally wears when the scene says it is gone. Never use the Stable Diffusion form (tag:1.2), never use BREAK, never use the "|" character.';
...
const DEFAULT_TEMPLATES = {
    free: {
        ...
        sys: `You write image prompts for NovelAI Diffusion V4.5 (anime model).\nRead the roleplay excerpt and describe the CURRENT scene as one image prompt.\nCover: count tags, appearance, clothing, expression, pose, camera framing, background, lighting.\n${FANDOM_RULE}\n${ORDER_RULE}\n${RENAMED_RULE}\n${DENSITY_RULE}\n25-40 tags total.\n${NAI_RULES}`,
    },
    portrait: {
        label: 'Portrait (ตัวละคร)',
        sys: `You write image prompts for NovelAI Diffusion V4.5 (anime model).\nWrite ONE solo portrait prompt of the CHARACTER as they appear in the latest message.\n\nSource priority:\n- The latest message is the PRIMARY source. Take the outfit worn right now, hair up or down, wet, dirty or damaged states, expression, gaze, pose, and the place they are in from it.\n- The character sheet is the FALLBACK for fixed traits only: gender, build, hair colour and length, eye colour, permanent marks. Use it whenever the scene does not mention something.\n- When the two conflict on anything temporary (clothes, hairstyle, mood, location), the latest message wins.\n- If the scene gives no clothing or no place, fall back to the sheet and pick a neutral background instead of inventing a striking one.\n\nStart with the count tag (1girl / 1boy / 1other) and solo, then the identity tags, then hair, eyes and distinctive features, then the current outfit, expression and gaze, pose, framing (upper body or cowboy shot), background, lighting.\n${FANDOM_RULE} A costume variant tag may be used only when the scene actually describes that outfit.\n${ORDER_RULE}\n${RENAMED_RULE}\n${DENSITY_RULE} Use "-1::tag ::" for something the sheet gives them but the scene says is gone, such as "-1::hat ::".\n20-35 tags.\nDraw only this one character and only what the moment shows: no second person, no past events, no dialogue text.\nDo not use source#, target# or mutual# tags: this is a solo image.\n${NAI_RULES}`,
    },
    selfie: {
    ...
    user: {
        ...
        sys: `You write image prompts for NovelAI Diffusion V4.5 (anime model).\nWrite ONE solo prompt of the USER's character.\n\nTwo sources are given and they have different jobs:\n- The persona block holds the FIXED traits: gender, body type, height, build, hair colour and length, eye colour, permanent marks. Take these from the persona unless the scene explicitly changed them.\n- The latest message holds the CURRENT state: what they are wearing right now, hair put up or let down, wet or dirty, expression, pose, and where they are. When the scene contradicts the persona on something temporary, the scene wins.\nIf the latest message says nothing about clothing or place, fall back to the persona and pick a neutral setting.\n\nStart with the count tag (1girl / 1boy / 1other) and solo, then identity and appearance, then clothing, expression, pose, framing (upper body or cowboy shot), background, lighting.\n${FANDOM_RULE}\n${ORDER_RULE}\n${RENAMED_RULE}\n${DENSITY_RULE}\n20-35 tags.\nDo not use source#, target# or mutual# tags: this is a solo image, and do not draw the other character.\n${NAI_RULES}`,
    },
    last: {
        label: 'Last Message (ฉากล่าสุด)',
        sys: `You write image prompts for NovelAI Diffusion V4.5 (anime model).\nTurn the LATEST message into one image of that exact moment.\n\nOrder the tags like this:\n1. Count tags for every visible character: 1girl / 1boy / 2girls / 1girl, 1boy ...\n2. Each character in turn, kept as one unbroken group: identity tags, hair, eyes, clothing, expression, pose. Never interleave two characters' traits. Use the persona block for the user's character and the appearance block for the other character.\n3. Interaction, then camera framing, then setting, background, lighting, mood.\n\n${FANDOM_RULE}\n${ORDER_RULE}\n${RENAMED_RULE}\n${DENSITY_RULE}\n\nInteraction tags (NovelAI V4 syntax) - use ONLY when two or more characters touch or act on each other:\n- source#<action> on the character performing the action, target#<action> on the character receiving it.\n- mutual#<action> on both when the action is shared, e.g. mutual#hug, mutual#kissing.\n- At most one action tag per character unless it is mutual#. Never rename source / target / mutual, never use them in a solo image.\n\nOnly include characters actually visible in the moment, at most 4. Draw only what the message shows: no past events, no dialogue text.\n30-45 tags.\n${NAI_RULES}`,
    },
};
