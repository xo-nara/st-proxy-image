...
export const NAI_DOCS = [
    {
        ...
        engine: 'nai',
        icon: 'fa-rocket',
        title: 'NovelAI V5 (อัปเดตใหม่)',
        body: `# NovelAI Diffusion V5

ออก 21 ส.ค. 2026 — สรุปเฉพาะจุดที่กระทบการใช้งานกับส่วนขยายนี้

## โมเดล
- \`nai-diffusion-5-full\` รุ่นเต็ม ใช้เป็นค่าเริ่มต้น
- \`nai-diffusion-5-curated\` รุ่นคัดกรอง เอนไปทาง SFW
- รุ่นเก่า V4.5 / V4 / V3 ยังเลือกได้จากรายการเดิม

ถ้า NovelAI ปล่อยชื่อโมเดลใหม่ พิมพ์ลงช่อง Model ได้เลย ไม่ต้องรออัปเดตส่วนขยาย

## แท็กใหม่ที่ใช้ได้จริง
- **ระดับรายละเอียด** \`low complexity\` / \`medium complexity\` / \`high complexity\` / \`ultra complexity\`
  ยิ่งสูงยิ่งเติมฉาก พร็อพ และตัวประกอบเข้ามา เลือกได้ทีละอันเท่านั้น
- **\`depthness\`** แยกระยะหน้า-หลังให้ชัด เหมาะกับฉากที่มีพื้นที่จริง ไม่จำเป็นกับภาพครึ่งตัวธรรมดา
- **ยุคของลายเส้น** \`meta:novel era\` ให้ลุคไลท์โนเวลสมัยใหม่ • \`meta:golden era\` ให้ลุคอนิเมะยุคคลาสสิก
- **กรอบแบบวิชวลโนเวล** \`visual novel art\` / \`visual novel cg\` / \`visual novel bg\` / \`visual novel sprite\` / \`visual novel chibi\`
- **ตัดพื้นหลัง** \`transparent background\` / \`has alpha\` / \`alpha transparency\`
  ใช้เมื่อต้องการตัวละครแบบตัดฉากออกเท่านั้น ห้ามใช้คู่กับการบรรยายสถานที่
- **\`attractive male\`** ช่วยให้ตัวละครชายออกมาดีขึ้น

## สิ่งที่ V5 ทำได้ดีขึ้น
- **ภาษาธรรมชาติ** เข้าใจประโยคบรรยายดีขึ้นมาก แต่แท็กยังใช้ได้เต็มที่ เทมเพลตแท็กเดิมจึงยังใช้ต่อได้
- **หน้าการ์ตูนหลายช่อง** ทำได้ในรอบเดียว ไม่จำกัดแค่ 2 ช่องเหมือนก่อน — โหมด Manga Panel ในส่วนขยายนี้เขียนใหม่ให้ใช้ความสามารถนี้แล้ว
- **ตัวหนังสือ** เรนเดอร์ได้แม่นขึ้นมาก ใส่ข้อความในเครื่องหมายคำพูดแล้วมันจะวาดตามนั้น
- **การจัดวางตัวละคร** ดีขึ้น แต่ระบบตำแหน่งแบบตารางยังต้องผ่าน char_captions ซึ่งเซิร์ฟเวอร์ SillyTavern ตัดทิ้ง จึงยังเข้าไม่ถึงจากที่นี่

## สิ่งที่ยังไม่มา
Vibe Transfer ยังไม่รองรับใน V5 (ทาง NovelAI ระบุว่ายังพัฒนาอยู่)
กล่อง Vibe ในส่วนขยายนี้เป็นพรีเซ็ตแท็ก ไม่ใช่ Vibe Transfer ตัวจริง จึงใช้ได้ตามปกติ

## ชุดค่าที่ส่วนขยายตั้งให้
กดปุ่ม "ใส่ค่าที่ NovelAI แนะนำ" เมื่อเลือกโมเดล V5 จะได้
- suffix: \`location, very aesthetic, masterpiece, high complexity, no text\`
- negative: ชุดของ V5 โดยเฉพาะ (ตัด artifact และ anatomy ที่พบบ่อย)
- sampler k_euler_ancestral, scheduler karras, steps 28, CFG 5`,
    },
    {
        id: 'artist',
        engine: 'nai',
...
- ฝั่ง Connection 1 (โมเดลข้อความที่เขียน prompt) มีฟิลเตอร์ของตัวเอง
  ...
  หรือข้าม Connection 1 ไปเลยด้วย \`/pxi raw=true <prompt ของคุณ>\``,
    },
    {
        ...
        engine: 'gpt',
        icon: 'fa-cube',
        title: 'เลือกโมเดล + ขนาดภาพ',
        body: `# เลือกโมเดล และขนาดภาพ
...
- \`gpt-image-1.5\` / \`gpt-image-1\` เก็บไว้เพื่อความเข้ากันได้กับของเดิมเท่านั้น
- \`gpt-image-1-mini\` เมื่อต้นทุนสำคัญกว่าคุณภาพ เช่นเจนจำนวนมากเพื่อคัดเลือก

\`input_fidelity\` ไม่มีผลกับ gpt-image-2 เพราะมันคืนภาพความละเอียดสูงอยู่แล้ว

## ขนาดภาพ (gpt-image-2)
ใส่ขนาดอะไรก็ได้ ขอแค่ผ่านครบทุกข้อ
- ด้านยาวสุดน้อยกว่า 3840px
- ทั้งสองด้านหารด้วย 16 ลงตัว
- อัตราส่วนด้านยาวต่อด้านสั้นไม่เกิน 3:1
...
- 3840x2160 (4K) ยังเป็นของทดลอง ถ้าติดกฎด้านยาวให้ปัดเป็น 3824x2144
...
รุ่นเก่า (1 / 1.5 / mini) รับแค่ \`1024x1024\`, \`1024x1536\`, \`1536x1024\`, \`auto\``,
    },
    {
        ...
        engine: 'gpt',
        icon: 'fa-pen-fancy',
        title: 'หลักการเขียน prompt',
        body: `# หลักการเขียน prompt

## โครงสร้าง
เรียงลำดับให้คงที่: ฉาก/พื้นหลัง → ตัวแบบ → รายละเอียดสำคัญ → ข้อจำกัด
บอกด้วยว่าเอาไปใช้ทำอะไร (โฆษณา ภาพประกอบ mockup) เพื่อให้โมเดลรู้ระดับความเนี้ยบที่ต้องการ

## รูปแบบ
ประโยคบรรยาย, JSON, คำสั่งเป็นข้อ ๆ หรือแท็ก ใช้ได้หมดตราบใดที่เจตนาชัด
เลือกแบบที่ดูแลง่ายที่สุด งานซับซ้อนให้แบ่งบรรทัดหรือใส่หัวข้อสั้น ๆ ดีกว่าเขียนยาวรวดเดียว

## ความสมจริง
อยากได้ภาพถ่ายจริง ให้ใส่คำว่า \`photorealistic\` ตรง ๆ
คำอย่าง real photograph / taken on a real camera / iPhone photo ก็ช่วยได้
สเปกกล้องละเอียด ๆ โมเดลตีความหลวม ๆ ใช้กำหนดโทนรวมได้แต่อย่าหวังผลเป๊ะ

## วัสดุและสื่อ
เจาะจงเรื่องพื้นผิวและสื่อ (ภาพถ่าย สีน้ำ เรนเดอร์ 3D) แล้วค่อยเติมคำเพิ่มคุณภาพเฉพาะจุด เช่น film grain, macro detail

## การไล่แก้
ค่อย ๆ แก้ทีละจุดดีกว่ายัดทุกอย่างในรอบเดียว
เริ่มจาก prompt ฐานสะอาด ๆ แล้วสั่งแก้ทีละอย่าง`,
    },
    {
        id: 'gpt_composition',
        engine: 'gpt',
        icon: 'fa-camera-retro',
        title: 'องค์ประกอบภาพ และคน',
        body: `# องค์ประกอบภาพ และการวางคน

## กล้องและแสง
- ระยะภาพและมุมกล้อง: close-up, wide, top-down, eye-level, low-angle
- แสงและอารมณ์: soft diffuse, golden hour, high-contrast

## ตำแหน่ง
ถ้าตำแหน่งสำคัญให้บอกตรง ๆ เช่น "logo top-right", "subject centered with negative space on left"
GPT-Image อ่านภาษาบอกตำแหน่งแบบธรรมดาได้ดี ใช้คำพูดปกติได้เลย ไม่ต้องใช้แท็ก

## ฉากใหญ่
ฉากกว้าง ฉากกลางคืน ฝน หรือนีออน ให้เพิ่มรายละเอียดเรื่องสเกล บรรยากาศ และสี
ไม่งั้นโมเดลจะเลือกความสมจริงของพื้นผิวแทนอารมณ์

## เรื่องคน
บอกสัดส่วนในเฟรม ส่วนของร่างกายที่เห็น ทิศทางสายตา และการจับต้องสิ่งของ
- "full body visible, feet included"
- "looking down at the open book, not at the camera"
- "hands naturally gripping the handlebars"
สามอย่างนี้ช่วยเรื่องสัดส่วนและท่าทางได้มากที่สุด`,
    },
    {
        id: 'gpt_edit',
        engine: 'gpt',
        icon: 'fa-scissors',
        title: 'ข้อจำกัด และการแก้ภาพ',
        body: `# ข้อจำกัด และการแก้ภาพ

## เขียนข้อห้ามตรง ๆ
no watermark, no extra text, no logos/trademarks

## สูตรแก้ภาพ
ใช้โครงสร้างสามส่วนเสมอ
- Change: อะไรเปลี่ยน
- Preserve: อะไรห้ามแตะ
- Constraints: ห้ามมีอะไร

สั่งว่า "change only X" คู่กับ "keep everything else the same" และ**ทวนรายการที่ห้ามเปลี่ยนทุกรอบ** เพื่อกันภาพเพี้ยนสะสม

## แก้แบบผ่าตัด
บอกด้วยว่าห้ามแตะความอิ่มสี คอนทราสต์ เลย์เอาต์ ลูกศร ป้ายกำกับ มุมกล้อง และวัตถุรอบข้าง

## endpoint แก้ภาพ
\`POST /v1/images/edits\` รับ input ได้ถึง 16 ภาพ ไฟล์ละไม่เกิน 50MB ใช้ multipart/form-data
- mask เป็น PNG ไม่เกิน 4MB ขนาดเท่าภาพแรก พื้นที่โปร่งใส = โซนที่ให้แก้
- \`input_fidelity\` high/low ใช้กับ gpt-image-1 และ 1.5 ขึ้นไป (ไม่มีใน mini)
- ส่วนขยายนี้ยังไม่รองรับ endpoint นี้ รองรับแค่ \`/v1/images/generations\``,
    },
    {
        id: 'gpt_text',
        engine: 'gpt',
        icon: 'fa-font',
        title: 'ตัวหนังสือในภาพ',
        body: `# ตัวหนังสือในภาพ

GPT-Image เก่งเรื่องตัวหนังสือมาก แต่ถ้าปล่อยหลวมมันจะแถตัวอักษรมั่ว ๆ มาให้ ต้องล็อกข้อความและตำแหน่งเสมอ
...
## ในส่วนขยายนี้
template ทุกโหมดสั่งห้ามใส่ตัวหนังสือในภาพไว้แล้ว ถ้าอยากได้ป้ายหรือข้อความจริง ๆ ให้เขียนสั่งเพิ่มในช่อง Extra instruction`,
    },
    {
        id: 'gpt_params',
        engine: 'gpt',
        icon: 'fa-sliders',
        title: 'พารามิเตอร์ และค่าใช้จ่าย',
        body: `# พารามิเตอร์ และค่าใช้จ่าย

## quality
- \`low\` เริ่มจากตรงนี้ถ้าเน้นเร็วหรือทำจำนวนมาก หลายกรณีคุณภาพพอแล้ว
- \`medium\` / \`high\` เทียบก่อนใช้จริงเมื่อเป็นตัวหนังสือเล็ก อินโฟกราฟิกแน่น ภาพใบหน้าโคลสอัพ งานแก้ที่ต้องรักษาหน้าคน และภาพความละเอียดสูง

## พารามิเตอร์อื่น
- \`n\` สร้างหลายภาพในคำขอเดียว (1-10) เหมาะกับการทำตัวเลือกมาเทียบ แต่คิดเงินตามจำนวนภาพจริง
- \`output_format\` png (ค่าเริ่มต้น) / jpeg / webp — jpeg เร็วกว่า png
...
## ค่าใช้จ่าย
คิดเป็น token ไม่ใช่ต่อภาพ ภาพออกแพงกว่าข้อความเข้าหลายเท่า
ตั้ง n สูง ๆ จึงกินเงินไวกว่าที่คิด`,
    },
    {
        id: 'gpt_extension',
        engine: 'gpt',
        icon: 'fa-plug',
        title: 'ใช้กับส่วนขยายนี้',
        body: `# ใช้ GPT-Image กับส่วนขยายนี้

## เปิดใช้
ตั้ง "ชุดพารามิเตอร์" ในหมวด ⑤ เป็น GPT-Image
GPT-Image ต่อผ่าน **Custom proxy เดียวกัน** ใน Connection 2 ไม่มีปลายทางแยก และไม่มีการเดาจากชื่อโมเดล

## template
ชุด template ของ GPT เขียนเป็นร้อยแก้วตามหลักการข้างต้นอยู่แล้ว ไม่ใช้แท็กแบบ NovelAI
สลับชุด template ได้ที่หมวด ③ (แยกจากชุดพารามิเตอร์ ไม่ผูกกัน)

## สไตล์
"สไตล์เริ่มต้น" ในหมวด ⑤ จะถูกต่อท้าย system prompt เป็นบรรทัด Style: ...
เลือกได้ 14 แบบ หรือเขียนเอง และเลือกตอนกดสร้างได้จาก popup

## Vibe
ชุดแท็กที่บันทึกไว้จะถูกใส่เป็นบรรทัด Recurring look ใน system (ไม่ยัดลง prompt เหมือนฝั่ง NovelAI)

## เมื่อโดนกรองเนื้อหา
popup จะให้แก้ prompt แล้วยิงใหม่ได้ทันที ลองแก้เฉพาะคำที่สุ่มเสี่ยง และลองตั้ง moderation เป็น low`,
    },
    {
        id: 'settings',
        engine: 'nai',
...
export function docsToMarkdown(engine = null) {
    const list = engine ? NAI_DOCS.filter(doc => doc.engine === engine) : NAI_DOCS;
    const name = engine === 'gpt' ? 'GPT-Image' : engine === 'nai' ? 'NovelAI' : 'NovelAI + GPT-Image';
    const source = engine === 'gpt' ? 'OpenAI Cookbook' : engine === 'nai' ? 'docs.novelai.net' : 'docs.novelai.net และ OpenAI Cookbook';
    const header = `# ${name} Cheatsheet — Proxy Image Gen\n\nรวบรวมจาก ${source}\n`;
    return [header, ...list.map(doc => doc.body)].join('\n\n---\n\n');
}
